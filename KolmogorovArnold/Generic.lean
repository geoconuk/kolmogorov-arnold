...
-/
import KolmogorovArnold.Density
import KolmogorovArnold.StrictlyIncreasing
import Mathlib.Topology.ContinuousMap.SecondCountableSpace

/-!
# The Baire step: a generic tuple approximates every function in one step

Layer 3 of the Kolmogorov–Arnold development. Layer 1 made the approximation sets `U_f` open and
Layer 2 made them dense, for every `f` in the closed unit ball of `C(Iⁿ, ℝ)`. That ball is
separable, so the intersection of the `U_h` over a countable dense set of `h` is a dense `Gδ`
in the complete metric space of tuples (Baire); so is the set of tuples with strictly increasing
components (Layer 1), and two dense `Gδ`s meet. A tuple `ψ` in the intersection approximates
*every* continuous `f` in one step, with `‖g‖` and the error both proportional to `‖f‖`: this is
Hedberg's Lemma 3, and it is the input to the iteration of Layer 4.
...
Result selected by George A. Constantinides as Layer 3 of the development. Lean statements
and proofs machine-generated. This is a support module: its statement is not
