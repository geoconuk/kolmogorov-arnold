...
`KolmogorovArnold.*` module and fails the build unless its transitive axiom dependencies are
...
namespace KolmogorovArnold.Meta
...
def auditedRoot : Name := `KolmogorovArnold

/-- Is `m` a module belonging to this library? -/
def isAuditedModule (m : Name) : Bool :=
  m == auditedRoot || auditedRoot.isPrefixOf m

/-- Audit every declaration in every imported `KolmogorovArnold.*` module, and throw an error
listing any declaration that depends on an axiom outside `allowedAxioms`.

Intended to be invoked exactly once, from `KolmogorovArnold/Audit.lean`, which imports the
