...
-/
import KolmogorovArnold.Generic
...
Layer 4 of the Kolmogorov–Arnold development. Layer 3 produced a tuple `ψ` of strictly increasing
inner functions with a one-step approximation for every `f`: an outer `g` with `‖g‖ ≤ c ‖f‖`
and residual `‖f - ∑_q g ∘ y_q‖ ≤ θ ‖f‖`, `θ < 1`. Iterating on the residual gives outer
functions `g_j` with `‖g_j‖ ≤ c θ^j ‖f‖`, whose sum converges in the Banach space of bounded
continuous functions on the line; the superposition operator is a bounded linear map, so it
passes through the sum, and the images telescope to `f`. Hence every continuous `f` on the cube
