...
Layer 0 of the Kolmogorov–Arnold development. The inner sums of the superposition theorem
have the shape `y_q(x) = ∑_l λ_l ψ_q (x_l)`, and the constants `λ_1, …, λ_n` must be
positive and linearly independent over `ℚ`: rational independence is what stops two distinct
points of the cube from being merged by every one of the `2n + 1` inner sums at once.
...
Lean statement and proof machine-generated. The statement has had a best-effort read; the
proof has not been read.
