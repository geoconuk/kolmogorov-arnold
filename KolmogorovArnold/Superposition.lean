...
-/
import KolmogorovArnold.InnerSpace
...
Layer 1 of the Kolmogorov–Arnold development. Given constants `λ_1, …, λ_n` and a tuple
`ψ = (ψ_0, …, ψ_{2n})` of inner functions on `I = [0, 1]`, the superposition of an outer
function `g : ℝ → ℝ` is
...
- J.-P. Kahane, *Sur le théorème de superposition de Kolmogorov*, J. Approx. Theory 13
  ...
  is why Layer 2 will assume `‖f‖ ≤ 1`.

## Provenance

Definitions and statements selected by George A. Constantinides as Layer 1 of the
development, from a reading of Hedberg's and Kahane's proofs. Lean statements and proofs
