...
while IFS= read -r f; do
  files+=("$f")
done < <(find KolmogorovArnold -name '*.lean' -not -path 'KolmogorovArnold/Meta/*' -not -name 'Audit.lean' | sort)

# The *result modules* are the ones KolmogorovArnold.lean names directly; check-imports.sh lets
...
done < <(grep -oE '^import[[:space:]]+KolmogorovArnold\.[A-Za-z0-9_.]+' KolmogorovArnold.lean | awk '{print $2}')
...
if [[ ${#result_files[@]} -eq 0 ]]; then
  ...
  echo "note: no result files under KolmogorovArnold/ yet; checking infrastructure only"
fi

# KolmogorovArnold/Audit.lean is the module that *runs* the audit, so its own declarations are
# not imported into the audited environment and would escape the check. It must stay
# empty of results.
forbid KolmogorovArnold/Audit.lean '^[[:space:]]*(theorem|lemma|def|instance|abbrev)[[:space:]]' \
