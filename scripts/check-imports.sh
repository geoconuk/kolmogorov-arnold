#!/usr/bin/env bash
# Every module under KolmogorovArnold/ must be reachable from the root KolmogorovArnold.lean.
#
# The axiom audit in KolmogorovArnold/Audit.lean can only see declarations that are
...
# only the roof needs naming in KolmogorovArnold.lean; the audit sees the rest through it. The
...
root="KolmogorovArnold.lean"
status=0

# The KolmogorovArnold modules a given file imports.
imports_of() {
  grep -oE '^import[[:space:]]+KolmogorovArnold\.[A-Za-z0-9_.]+' "$1" | awk '{print $2}' || true
}
...
is_exempt() {
  case "$1" in
    KolmogorovArnold.Audit | KolmogorovArnold.Meta.*) return 0 ;;
    *) return 1 ;;
  esac
}
...
while IFS= read -r file; do
  # KolmogorovArnold/Foo/Bar.lean -> KolmogorovArnold.Foo.Bar
...
done < <(find KolmogorovArnold -name '*.lean' | sort)
...
if [[ $status -eq 0 ]]; then
  echo "import check passed: every module under KolmogorovArnold/ is reachable from $root"
fi
exit $status
